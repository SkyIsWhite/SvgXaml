...
public static class ConverterLogic
{
    ...
    public static object ConvertSvgToObject(string filepath, ResultMode resultMode,
        WpfDrawingSettings wpfDrawingSettings, out string name, ResKeyInfo resKeyInfo)
    {
    ...
    public static IEnumerable<string> SvgFilesFromFolder(string folder, bool handleSubFolders = false)
    {
        try
        {
            if (handleSubFolders) return Directory.GetFiles(folder, "*.svg*", SearchOption.AllDirectories);
    ...
    internal static ResourceDictionary ConvertFilesToResourceDictionary(IEnumerable<string> files,
        WpfDrawingSettings wpfDrawingSettings, ResKeyInfo resKeyInfo)
    {
    ...
    public static IEnumerable<PathGeometry> GetPathGeometries(Drawing drawing)
    {
        ...
        handleDrawing = aDrawing =>
        {
            if (aDrawing is DrawingGroup)
                foreach (var d in ((DrawingGroup)aDrawing).Children)
                    handleDrawing(d);
            if (aDrawing is GeometryDrawing)
            {
                var gd = (GeometryDrawing)aDrawing;
                var geometry = gd.Geometry;
                if (geometry is PathGeometry) result.Add((PathGeometry)geometry);
            }
        };
    ...
    public static void SizeGeometry(PathGeometry pg, Size size)
    {
        if (size.Height > 0 && size.Height > 0)
        {
            ...
            var newGeo =
                new PathGeometry(sizeFigures.Concat(pg.Figures), pg.FillRule,
    ...
    private static void FixIds(XElement root)
    {
        ...
        {
            if (char.IsDigit(attr.Value.FirstOrDefault())) attr.Value = "_" + attr.Value;
    ...
    private static void InlineClipping(XElement drawingElement)
    {
        ...
        if (clipElement != null && clipElement.Parent.Name.LocalName == "DrawingGroup")
        {
            ...
            clipElement.Parent.Add(new XAttribute("ClipGeometry",
                string.Format(CultureInfo.InvariantCulture, "M{0},{1} V{2} H{3} V{1} H{0} Z", clipRect.Left,
    ...
    private static void CollapsePathGeometries(XElement drawingElement)
    {
        ...
        foreach (var pathGeometry in pathGeometries)
            if (pathGeometry.Parent != null && pathGeometry.Parent.Parent != null &&
    ...
    internal static Size? GetSizeFromDrawingGroup(DrawingGroup drawingGroup)
    {
        ...
        if (drawingGroup != null)
        {
            ...
            if (subGroup != null) return subGroup.ClipGeometry.Bounds.Size;
        }
