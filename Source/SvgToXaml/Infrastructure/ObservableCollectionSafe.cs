...
public class ObservableCollectionSafe<T> : INotifyCollectionChanged, INotifyPropertyChanged, IList<T>, IList,
    IReadOnlyList<T>, IDisposable
{
    ...
    public T this[int index]
    {
        ...
        set
        {
            WriteAccess(false, (ref object dummy) =>
            {
                ...
                _coll[index] = value;
                return new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Replace, originalItem, value,
                    index);
            });
    ...
    public void HandleSynchonizationCallback(IEnumerable collection, object context, Action accessMethod,
        bool writeAccess)
    {
    ...
    private void CopyFrom(IEnumerable<T> items)
    {
        if (items != null)
            ...
            {
                while (enumerator.MoveNext()) _coll.Add(enumerator.Current);
            }
    }
    ...
    public void Move(int oldIndex, int newIndex)
    {
        WriteAccess(false, (ref object dummy) =>
        {
            var removedItem = _coll[oldIndex];

            _coll.RemoveAt(oldIndex);
            _coll.Insert(newIndex, removedItem);
            return new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Move, removedItem, newIndex,
                oldIndex);
        });
        //_lock.EnterWriteLock();
        //try
    ...
    public void AddRange(IEnumerable<T> items)
    {
        ...
        {
            foreach (var item in items) _coll.Add(item);
    ...
    public void RemoveRange(IEnumerable<T> items)
    {
        WriteAccess(true, (ref object dummy) =>
        {
            foreach (var item in items) _coll.Remove(item);
            return new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset);
        });
    }
    ...
    object ICollection.SyncRoot =>
        throw new NotSupportedException("This ObservableCollection doesn't need external synchronization");
