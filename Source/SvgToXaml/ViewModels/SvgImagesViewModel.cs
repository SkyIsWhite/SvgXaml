...
public class SvgImagesViewModel : ViewModelBase
{
    ...
    public SvgImagesViewModel()
    {
        ...
        ContextMenuCommands.Add(new Tuple<object, ICommand>("Open Explorer",
            new DelegateCommand<string>(OpenExplorerExecute)));
    }
    ...
    private void OpenFolderExecute()
    {
        var folderDialog = new FolderBrowserDialog
            { Description = "Open Folder", SelectedPath = CurrentDir, ShowNewFolderButton = false };
    ...
    {
        var outFileName = Path.GetFileNameWithoutExtension(CurrentDir) + ".xaml";
        var saveDlg = new SaveFileDialog
        {
            AddExtension = true, DefaultExt = ".xaml", Filter = "Xaml-File|*.xaml", InitialDirectory = CurrentDir,
        ...
        if (saveDlg.ShowDialog() == DialogResult.OK)
        {
            ...
            var useComponentResKeys = false;
            string nameSpaceName = null;
            var nameSpace =
                Interaction.InputBox(
                    "Enter a NameSpace for using static ComponentResKeys (or leave empty to not use it)", "NameSpace");
            ...
            {
                namePrefix = Interaction.InputBox("Enter a namePrefix (or leave empty to not use it)", "Name Prefix");
    ...
    private void BuildBatchFile(string outFileName, ResKeyInfo compResKeyInfo)
    {
                ...
                null, MessageBoxButton.YesNoCancel) == MessageBoxResult.Yes)
        {
            ...
            var svgToXamlPath = Assembly.GetEntryAssembly().Location;
            ...
            {
                batchText +=
                    $" /useComponentResKeys=true /compResKeyNSName={compResKeyInfo.NameSpaceName} /compResKeyNS={compResKeyInfo.NameSpace}";
    ...
    private static string[] GetFilesMulti(string sourceFolder, string filters,
